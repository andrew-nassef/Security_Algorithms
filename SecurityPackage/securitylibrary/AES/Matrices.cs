...
namespace SecurityLibrary.AES
{
    public class Matrices
    {
        ...
        public static int[,] RijndaelMixColumnsMatrix = new int[,] {
        {2, 3, 1, 1 },
        {1, 2, 3, 1 },
        {1, 1, 2, 3 },
        {3, 1, 1, 2 }
        ...
        public static int mapToIndex(int i)
        {
            // 0 : 48, A : 65, a : 97
            if (i <= 58 && i >= 48)
                return i - 48;
        ...
        static public string replaceFromSBox(string s)
        {
            if (s.Length == 1) s = "0" + s;
                ...
                for (int j = 0; j < matrix1.GetLength(1); j++)
                {
                    for (int k = 0; k < matrix2.GetLength(1); k++)
                    {
                        ...
                        if (result[i, k] == 0)
                            ...
                            result[i, k] = Convert.ToInt32(XOR(Convert.ToString(multpliedValue, 2), 
                                Convert.ToString(result[i, k], 2)), 2);
                    }
                }
            }

            string[,] ans = new string[result.GetLength(0), result.GetLength(1)];
            for(int i = 0; i < result.GetLength(0); i++)
            {
                for(int j = 0;j < result.GetLength(1); j++)
                {
                    ans[i, j] = (result[i, j]).ToString("X");
                }
            }
            return ans;
        }
